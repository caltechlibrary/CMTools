...
var w3c_slidy = {
  ...
  add_toolbar: function () {
    ...
    if (this.ns_pos || !this.ie6) {
    ...
    } // IE6 so need to work around its poor CSS support
    else {
      ...
      contents.setAttribute(
        "title",
        this.localize("table of contents".localize),
      );
      contents.innerHTML = this.localize("contents?");
  ...
  check_location: function () {
    ...
    if (w3c_slidy.slide_number > 0 && (hash == "" || hash == "#")) {
      ...
      hash.length > 2 && hash != "#(" + (w3c_slidy.slide_number + 1) + ")"
    ) {
  ...
  notify_observers: function () {
    ...
    for (var i = 0; i < this.observers.length; ++i) {
      this.observers[i](
        this.slide_number + 1,
        this.find_heading(slide).innerText,
        location.href,
      );
    }
  },
  ...
  push_hash: function (hash) {
    ...
    doc.write(
      '<html><head><script type="text/javascript">window.parent.w3c_slidy.on_frame_loaded(\'' +
  ...
  previous_slide: function (incremental) {
    if (!w3c_slidy.view_all) {
      ...
      if (
        (incremental || w3c_slidy.slide_number == 0) &&
        w3c_slidy.last_shown != null
      ) {
        w3c_slidy.last_shown = w3c_slidy.hide_previous_item(
  ...
  next_slide: function (incremental) {
    if (!w3c_slidy.view_all) {
      ...
      if (
        incremental || w3c_slidy.slide_number == w3c_slidy.slides.length - 1
      ) {
        w3c_slidy.last_shown = w3c_slidy.reveal_next_item(w3c_slidy.last_shown);
      }

      if (
        (!incremental || w3c_slidy.last_shown == null) &&
  ...
  adjust_object_dimensions: function (width, height) {
    for (var i = 0; i < w3c_slidy.objects.length; i++) {
      ...
      if (
        ...
        if (
          obj.initialWidth &&
          obj.initialWidth.charAt(obj.initialWidth.length - 1) == "%"
        ) {
          var w = parseInt(
            obj.initialWidth.slice(0, obj.initialWidth.length - 1),
          );
        ...
        if (
          ...
          var h = parseInt(
            obj.initialHeight.slice(0, obj.initialHeight.length - 1),
          );
  ...
  key_down: function (event) {
    ...
    if (
      w3c_slidy.is_shown_toc() && key != 9 && key != 16 && key != 38 &&
      key != 40
    ) {
      w3c_slidy.hide_table_of_contents(true);
    ...
    if (key == 34) { // Page Down
      ...
      var alert_msg = (w3c_slidy.mouse_click_enabled ? "enabled" : "disabled") +
        " mouse click advance";
  ...
  next_incremental_item: function (node) {
    ...
    for (;;) {
      ...
      if (node.nodeType == 1) { // ELEMENT
        ...
        if (
          w3c_slidy.has_class(node, "incremental") &&
        ...
        if (
          w3c_slidy.has_class(node.parentNode, "incremental") &&
  ...
  mouse_button_click: function (e) {
    ...
    if (
      Math.abs(e.clientX - w3c_slidy.mouse_x) +
    ...
    if (
      w3c_slidy.mouse_click_enabled && leftclick &&
      !w3c_slidy.special_element(target) &&
      !target.onclick
    ) {
      w3c_slidy.next_slide(true);
