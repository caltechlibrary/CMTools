...
async function main() {
    ...
    const date = new Date();
    const releaseDate = `${date.getFullYear()}-${date.toLocaleDateString('en-US', { month: '2-digit'})}-${date.toLocaleDateString('en-US', {day: '2-digit'})}`;
    const version = (codemeta['version'] === undefined) ? '0.0.0': codemeta['version'];
